        ...
        if data and data.status not in ['kicked', 'left'] and not data.user.is_bot:
            ...
            if info is None:
                ...
                if outn == 100:
                    ...
                    outn = 0
                continue
            elif info.house is Houses.NONE.value and (info.level is None or info.profession is Professions.NONE.value):
                n += 1
                if print_n:
                    icon = icon + '🖤'
                    outn += 1
...
def whois_group(bot, update, args=None):
    ...
    if args is not None and len(args)== 1:
        if args[0].isdigit():
            ...
            if user is None:
                output = "❌ No he encontrado el grupo que buscas."
                bot.sendMessage(
                    chat_id=chat_id,
                    text=output,
                    parse_mode=telegram.ParseMode.MARKDOWN
